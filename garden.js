...
function renderGarden(gameState) {
    ...
    // Clear any existing growth-check interval before starting a new one
    ...
    container.innerHTML = `
        <div class="max-w-6xl mx-auto space-y-6 animate-entry">
            <div class="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                ...
                <div class="mt-4 md:mt-0" id="garden-unlock-btn-area">
                    ${_getUnlockButtonHtml(gameState)}
                </div>
            </div>
                ...
                <div class="space-y-6">
                    <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                        <h3 class="font-bold text-slate-700 mb-3 text-sm uppercase tracking-wide flex justify-between">
                            ...
                            <span class="text-xs font-normal text-slate-400">Tap to select</span>
                        </h3>
    ...
    _renderPlots(gameState);
    _renderSeedInventory(gameState);
    _renderTeaInventory(gameState);

    // Start one growth-check interval tied to this render
    _gardenIntervalId = setInterval(() => {
        // Only run while the garden tab is visible
        ...
        if (changed) {
            ...
            _updateGrowthBars(gameState);
        }
    }, 1000);
}

function _getUnlockButtonHtml(gameState) {
...
function _renderPlots(gameState) {
    ...
    gameState.garden.plots.forEach(plot => {
        ...
        if (!plot.unlocked) {
            ...
            div.onclick = () => showToast('Unlock this plot for 5,000 Gold!', 'info');

        } else if (plot.plant) {
            const seedData = GARDEN_ITEMS_DATABASE.seeds.find(s => s.id === plot.plant.seedId);

            if (plot.plant.ready) {
                div.className += ' bg-amber-50 border-amber-300 shadow-md';
                div.style.transform = 'scale(1.05)';
                div.innerHTML = `
                    <div class="text-3xl animate-bounce mb-1">${seedData ? seedData.emoji : '🌿'}</div>
                    <div class="text-xs font-bold text-amber-600 uppercase">Ready!</div>
                    <div class="text-[10px] text-amber-500">Tap to harvest</div>
                `;
                div.onclick = () => _harvestPlot(plot.id, gameState);
            } else {
            ...
            div.onclick = () => _handlePlotClick(plot.id, gameState);
        }
...
function _updateGrowthBars(gameState) {
    const plotEls = document.querySelectorAll('#garden-plots .garden-plot');
    const now = Date.now();

    gameState.garden.plots.forEach((plot, index) => {
        if (!plot.unlocked || !plot.plant || plot.plant.ready) return;
        const el = plotEls[index];
        if (!el) return;
...
function _renderSeedInventory(gameState) {
    ...
    keys.forEach(id => {
        ...
        const selected = selectedSeedId === id;
        el.className = `p-2 rounded-lg border cursor-pointer transition-all flex flex-col items-center text-center relative ${selected ? 'bg-green-100 border-green-400 ring-2 ring-green-200' : 'bg-slate-50 border-slate-200 hover:border-green-300'}`;
        el.onclick = () => {
    ...
    keys.forEach(id => {
        ...
        container.innerHTML += `
            <div class="p-2 rounded-lg bg-white border border-slate-100 shadow-sm flex flex-col items-center text-center relative group">
                ...
                <div class="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-36 bg-slate-800 text-white text-[10px] p-2 rounded pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-10">
                    ${data.desc}
                </div>
            </div>
        `;
...
function _handlePlotClick(plotId, gameState) {
    if (!selectedSeedId) {
...
function _harvestPlot(plotId, gameState) {
    ...
    if (typeof gameState.checkAchievements === 'function') {
        const unlocked = gameState.checkAchievements();
        unlocked.forEach(a => showToast(`Achievement unlocked: ${a.name}! 🏆`, 'success'));
    }

    showToast(`Harvested ${tea ? tea.name : 'Tea'}! ✨`, 'success');
    saveGame(gameState);
    _renderPlots(gameState);
    _renderTeaInventory(gameState);
}
